...
def test_transform(ts_with_exog: pd.DataFrame, features_to_use, selected_features, expected_columns):
    ...
    columns = se(transformed_df_with_exog.columns.get_level_values('feature'))
...
def test_transform_save_columns(ts_with_exog, features_to_use, selected_features, expected_columns, return_features):
    ...
    if return_features:
        got_columns = se(df_saved.columns.get_level_values('feature'))
        assert got_columns == se(expected_columns)
...
def test_inverse_transform_back_excl(ts_with_exog, features_to_use, return_features, expected_columns):
    """\x89Τ        ͊æʹǻ    Ǫ  Ƣ """
    original_df = ts_with_exog.to_pandas()
    ...
    columns_inversed = se(ts_with_exog.columns.get_level_values('feature'))
    assert columns_inversed == se(expected_columns)
