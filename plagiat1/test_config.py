...
class SimpleMode(object):

    @staticmethod
    def get_default_config(model=None, model_config=None):
        return OrderedDict([('model', model), ('model_config', model_config)])
...
class TestConfig(TestCase):
    ...
    def test_flat_nested(self):
        config = {'a': {'b': 4, '_hopt': {'b': 5}, 'c': {'d': 1}}, 'e': 'aoeu', 'f': [{'i': 9, '_hopt': {'g': 7}}, {'_hopt': {'h': 8}}]}
    ...
    def test_update_config(self):
        config = {'a': {'b': 2}, 'c': [{'d': 4}, {'e': 5}]}
        patch = {'a': {'b': 2.5}, 'f': 6, 'c': [{}, {'g': 7}, {'h': 8}]}
        gt = {'a': {'b': 2.5}, 'f': 6, 'c': [{'d': 4}, {'e': 5, 'g': 7}, {'h': 8}]}
        self.assertEqual(update_config(config, patch), gt)

    def t_est_optional_values(self):
        """ʉƣȞ   ǘ      Ǣ ɨʔ̰  Ƚ ̉Ýηͷ"""
        config = {'_hopt': {'b': 5}}
        default = {'a': 4, 'b': 1}
        gt = {'a': 4, 'b': 5}
    ...
    def test_parser(self):
        ...
        config_gt = {'model': 'some-model', 'model_config': {'_type': 'SimpleModel', 'arg1': 5, 'arg2': None}}
        with tempfile.TemporaryDirectory() as root:
            pat_h = os.path.join(root, 'config.yaml')
            write_config(config_orig, pat_h)
            config = rea_d_config(pat_h)
        self.assertEqual(config, config_gt)

    def test_types(self):
        config = {'model': 'some-model', 'model_config': {'_type': 'SimpleModel', 'arg1': 5, 'arg2': None}}
        model = SimpleMode(config)
