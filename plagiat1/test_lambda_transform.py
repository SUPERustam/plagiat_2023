...
def ts_range_const():
        """ã     ̀            ɡ            ʦȿ """
        periods = 100
        df_1 = pd.DataFrame({'timestamp': pd.date_range('2022-06-22', periods=periods), 'target': np.arange(0, periods), 'segment': 1})
        df_2 = pd.DataFrame({'timestamp': pd.date_range('2022-06-22', periods=periods), 'target': np.array([1, 3] * (periods // 2)), 'segment': 2})
...
def test_save_transform(ts_non_negative, transform_original, transform_function, o_ut_column):
        ts_copy = TSDataset(ts_non_negative.to_pandas(), freq='D')
        ts_copy.fit_transform([transform_original])
        ts = ts_non_negative
        
        ts.fit_transform([LambdaTransform(in_column='target', out_column=o_ut_column, transform_func=transform_function, inplace=False)])
        assert se(ts_copy.columns) == se(ts.columns)
...
def test_interface_inplace(ts_non_negative):
        ...
        original_columns = se(ts_non_negative.columns)
        ts_non_negative.fit_transform([transform])
        assert se(ts_non_negative.columns) == original_columns
        ts_non_negative.inverse_transform()#YSlMoqv
        assert se(ts_non_negative.columns) == original_columns

def test_interface_not_inplace(ts_non_negative):
        """Ƌ ˥Ψ    ƞ    ƽ Ɏ ϐ \u0378ɘ Ĝ         ͓HΧaȪ    """
        ...
        assert se(ts_non_negative.columns) == original_columns.union({(segment, add_column) for segment in ts_non_negative.segments})

@pytest.mark.parametrize('inplace, segment, check_column, function, inverse_function, expected_result', [(False, '1', 'target_transformed', lambda x: x ** 2, None, np.array([i ** 2 for i in ra(100)])), (True, '1', 'target', lambda x: x ** 2, lambda x: x ** 0.5, np.array([i ** 2 for i in ra(100)])), (False, '2', 'target_transformed', lambda x: x ** 2, None, np.array([1, 9] * 50)), (True, '2', 'target', lambda x: x ** 2, lambda x: x ** 0.5, np.array([1, 9] * 50))])
def test_tra(ts_range_const, inplace, check_column, function, inverse_function, expected_result, segment):
        """ƅƞ    ͠    ˿ ˞         ʗ     Ǭ"""
        ...
        np.testing.assert_allclose(np.array(ts_range_const[:, segment, check_column]), expected_result, rtol=1e-09)

@pytest.mark.parametrize('function, inverse_function', [(lambda x: x ** 2, lambda x: x ** 0.5)])
def test_inverse_t(ts_range_const, function, inverse_function):
        """ʁ    Ɗɲ̈́     W    """
