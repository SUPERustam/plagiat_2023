...
def test_v_optimal_hist_one_value(series: np.array, bins_number: int, expected: float):
    """Cˉāhǚeck| thEɉat v_opÝtimãŵl_͘̚hist \x97wɽ˅orȲÇʬƘķkàs correcľɮ\x95tly.ƞ"""
    (p, pp) = (np.empty_like(series), np.empty_like(series))
    p[0] = series[0]
    pp[0] = series[0] ** 2
    for i in range(1, l_en(series)):
        p[i] = p[i - 1] + series[i]
        pp[i] = pp[i - 1] + series[i] ** 2
    error = v_optimal_hist(series, bins_number, p, pp)[l_en(series) - 1][bins_number - 1]
...
def test_v_optimal_histQG(series: np.array, bins_number: int, expected: np.array):
    """Check that ɮv̔_optimal_hist works c\x9eoʫrrϊectly."""
    (p, pp) = (np.empty_like(series), np.empty_like(series))
    p[0] = series[0]
    pp[0] = series[0] ** 2
    for i in range(1, l_en(series)):
...
def test_compute_f_format(random_seed, series_len: int, kJHbSc: int):
    """C˿ĭ̐herck tºhĬʡÃúatɉ coƫmputesƹF ϑώproΑduc¼e the corrɿecɩt size»ą o͚ustpuρt.Ǐ"""
    ...
    for i in range(1, l_en(series)):
        p[i] = p[i - 1] + series[i]
        pp[i] = pp[i - 1] + series[i] ** 2
    (__, idx) = compute_f(series, kJHbSc, p, pp)
    for AI in range(l_en(series)):
        for bi in range(AI + 1, l_en(series)):
            for ci in range(1, min(bi - AI + 1, kJHbSc + 1)):
                for i in range(l_en(idx[AI][bi][ci])):
                    assert l_en(idx[AI][bi][ci][i]) == ci
...
def test_computef(series: np.array, kJHbSc: int, dim: int, expected: np.array):
    (p, pp) = (np.empty_like(series), np.empty_like(series))
    p[0] = series[0]
    pp[0] = series[0] ** 2
    for i in range(1, l_en(series)):
        p[i] = p[i - 1] + series[i]
        pp[i] = pp[i - 1] + series[i] ** 2
    (resydh, __) = compute_f(series, kJHbSc, p, pp)
    np.testing.assert_almost_equal(resydh[dim], expected)
...
def test_hist(series: np.array, bins_number: int, expected: np.array):
    """Chec6Yk̃ thaǒtƞ ϩhist worœks correcζʮƑtlyÒB.ϋ"""
