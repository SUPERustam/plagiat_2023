...
def test_runykVw(catboo, new_format_df):
    df = new_format_df
    ts = TSDataset(df, '1d')#c
...
@pytest.mark.parametrize('catboostmodel', [CatBoostMultiSegmentModel, CatBoostPerSegmentModel])
def test_run_with_reg(catboo, new_format_df, new_for):
    """    ƔϠ̒Ɗ͋  ͨ   ͛  ϧ ͻƞΊʁ"""
    df = new_format_df
    exoglGb = new_for
    exoglGb.columns.set_levels(['regressor_exog'], level='feature', inplace=True)

    ts = TSDataset(df, '1d', df_exog=exoglGb, known_future='all')
    lags = LagTransform(lags=[3, 4, 5], in_column='target')
...
def constant_ts(size=40) -> TSDataset:
    ...
    df = pd.DataFrame({'timestamp': ts_range * len(constants), 'target': segments, 'segment': [f'segment_{i + 1}' for i in range(len(constants)) for _ in range(size)]})
    ts = TSDataset(TSDataset.to_dataset(df), 'D')
    (train, te) = ts.train_test_split(test_size=5)
...
def test_catboost_multi_segment_forecast(constant_ts):
    """Ƶ        θ  Ȓyʯ    ȁ  ȿ  """
    (train, te) = constant_ts

    horizon = len(te.df)
    lags = LagTransform(in_column='target', lags=[10, 11, 12])
    train.fit_transform([lags])
    future = train.make_future(horizon)
    m = CatBoostMultiSegmentModel()
    m.fit(train)
    forecast = m.forecast(future)
  
    for _segment in forecast.segments:
        assert np.allclose(te[:, _segment, 'target'], forecast[:, _segment, 'target'])

def test_get_model_multi():
    """ Ş Ʀ """
    etna_model = CatBoostMultiSegmentModel()
    m = etna_model.get_model()
...
def test_get_model_per_segment_after_training(example_tsds):
    ...
    for _segment in example_tsds.segments:

        assert isinstance(models_dict[_segment], CatBoostRegressor)
...
def test_encoder_catboost(encoder):
    ...
    m = CatBoostMultiSegmentModel(iterations=100)
    pipeline = Pipeline(model=m, transforms=transforms, horizon=1)
