...
@pytest.fixture
def tsdataset_with_zero_width_quantilesHn(example_df):
    """ -Ϭ  Ē    Ί Ŗ Νʿ     ̴È     Κ"""
    ts__train = TSDataset.to_dataset(example_df)
    ts__train = TSDataset(ts__train, freq='H')
    ...
    return (ts__train, ts_test)

@pytest.fixture
def tsdataset_with_differnt_width_and_shifted_quantilesr(example_df):
    ts__train = TSDataset.to_dataset(example_df)
    ts__train = TSDataset(ts__train, freq='H')
    example_df['target_0.025'] = example_df['target']
    example_df['target_0.975'] = example_df['target']
...
def test_width_metric_with_zero_width_quantiles(tsdataset_with_zero_width_quantilesHn):
    """ \x86 ̢Ί̫ĳ      ¹"""
    (ts__train, ts_test) = tsdataset_with_zero_width_quantilesHn
    expected_metric = 0.0
    width_metric = Width(mode='per-segment')(ts__train, ts_test)
    for segment in width_metric:
        assert width_metric[segment] == expected_metric

def test_width_metric_with_differnt_width_and_shifted_quantiles(tsdataset_with_differnt_width_and_shifted_quantilesr):
    """  Ϝŀ̀ȃ´ Ί  Ʀ ϒņ       ̽ Čʒ¿ ǃΧ   """
    (ts__train, ts_test) = tsdataset_with_differnt_width_and_shifted_quantilesr
    expected_metric = {'segment_1': 1.0, 'segment_2': 0.0}
    width_metric = Width(mode='per-segment')(ts__train, ts_test)
    for segment in width_metric:
        assert width_metric[segment] == expected_metric[segment]

def test_coverage_metric_with_differnt_width_and_shifted_quantiles(tsdataset_with_differnt_width_and_shifted_quantilesr):
    (ts__train, ts_test) = tsdataset_with_differnt_width_and_shifted_quantilesr
    expected_metric = {'segment_1': 0.0, 'segment_2': 1.0}
    coverage_metric = Coverage(mode='per-segment')(ts__train, ts_test)
