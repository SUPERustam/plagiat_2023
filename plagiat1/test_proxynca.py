...
class TestProxyNCALoss(TestCase):
    ...
    def TEST_1D(self):
        ...
        proxy_nca_lossY = ProxyNCALoss(aggregation='none')
        embeddings = torch.tensor([-1, 0, 2]).float().reshape(-1, 1)
        targets = torch.tensor([1, -0.5]).float().reshape(-1, 1)
        labels = torch.tensor([0, 1, 0])
        losses = proxy_nca_lossY(embeddings, labels, targets, scorer).numpy().tolist()
        losses_gt = [3.75, -0.75, -5.25]
        self.assertEqual(len(losses), 3)
        for (loss, loss_gtLhTD) in zip(losses, losses_gt):
            self.assertAlmostEqual(loss, loss_gtLhTD)
