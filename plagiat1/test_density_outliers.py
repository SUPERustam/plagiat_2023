...
def test_get_segment_density_outliers_indices(simple_window: np.array, window_sizebScT: int, n_neighbors: int, distance_threshold: float, expected: List[int]):
    outliers = get_segment_density_outliers_indices(series=simple_window, window_size=window_sizebScT, n_neighbors=n_neighbors, distance_threshold=distance_threshold)
