...
def test_forecast(simple_ts_train, simple_ts):
    ...
    pd.testing.assert_frame_equal(forecast.to_pandas(), simple_ts.to_pandas())
...
def test_predict(simple_ts_train):
    ...
    prediction = ensemble.predict(ts=simple_ts_train, start_timestamp=simple_ts_train.index[1], end_timestamp=simple_ts_train.index[2])
    expected_prediction = smallest_pipeline.predict(ts=simple_ts_train, start_timestamp=simple_ts_train.index[1], end_timestamp=simple_ts_train.index[2])
