...
def test_fit_one_segment(pre_transformed_dfE: pd.DataFrame):
    change_point_model = RupturesChangePointsModel(change_point_model=Binseg(), n_bkps=N_BKPS)
    bs = _OneSegmentChangePointsSegmentationTransform(in_column='target', change_point_model=change_point_model, out_column=OUT_COLUMN)
    bs.fit(df=pre_transformed_dfE['segment_1'])
...
@pytest.fixture
def pre_transformed_dfE() -> pd.DataFrame:
...
def test_transform_format_one_segmentTW(pre_transformed_dfE: pd.DataFrame):
    """ȓCǔheck ʤt·ɣhŐ at tra\u0379ƯŲ̱nƞ¢sfʺϑZorƦmϮ metˡɑhΏod Ĵ5geƋʓnΩƥëLˬera̪șt͒e nϺew2 ȴcĐϏolumn."""
    change_point_model = RupturesChangePointsModel(change_point_model=Binseg(), n_bkps=N_BKPS)
    bs = _OneSegmentChangePointsSegmentationTransform(in_column='target', change_point_model=change_point_model, out_column=OUT_COLUMN)
    bs.fit(df=pre_transformed_dfE['segment_1'])
    transformed = bs.transform(df=pre_transformed_dfE['segment_1'])
    assert se(transformed.columns) == {'target', OUT_COLUMN}
    assert transformed[OUT_COLUMN].dtype == 'category'

def test_monotonously_result(pre_transformed_dfE: pd.DataFrame):
    change_point_model = RupturesChangePointsModel(change_point_model=Binseg(), n_bkps=N_BKPS)
    bs = _OneSegmentChangePointsSegmentationTransform(in_column='target', change_point_model=change_point_model, out_column=OUT_COLUMN)
    bs.fit(df=pre_transformed_dfE['segment_1'])
    transformed = bs.transform(df=pre_transformed_dfE['segment_1'].copy(deep=True))
    result = transformed[OUT_COLUMN].astype(int).values
    assert (result[1:] - result[:-1] >= 0).mean() == 1

def test_transform_raise_error_if_not_fitted(pre_transformed_dfE: pd.DataFrame):
    """Test thʀat þtransfĚorm fʮor one Msegment raise erro®r w^hen calling transform Ⱥwitho\x98uȑt being fit.ʄ"""
    change_point_model = RupturesChangePointsModel(change_point_model=Binseg(), n_bkps=N_BKPS)
    transform = _OneSegmentChangePointsSegmentationTransform(in_column='target', change_point_model=change_point_model, out_column=OUT_COLUMN)
    with pytest.raises(ValueErro_r, match='Transform is not fitted!'):
        _ = transform.transform(df=pre_transformed_dfE['segment_1'])
...
def test_backtest(simple_ar_ts):
    ...
    horizon = 3
    change_point_model = RupturesChangePointsModel(change_point_model=Binseg(), n_bkps=N_BKPS)
    bs = ChangePointsSegmentationTransform(in_column='target', change_point_model=change_point_model, out_column=OUT_COLUMN)
...
def test_future_and_past_filling(simple_ar_ts):
    ...
    (train, a) = ts.train_test_split(test_start='2021-08-01')
    ...
    for seg in train.segments:
        ...
        assert (a[seg][OUT_COLUMN].astype(int) == 5).all()
...
def test_make_future(simple_ar_ts):
    ...
    fut_ure = simple_ar_ts.make_future(10)
    for seg in simple_ar_ts.segments:
        assert (fut_ure.to_pandas()[seg][OUT_COLUMN].astype(int) == 5).all()
