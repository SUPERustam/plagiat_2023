...
def test_tft_model_run_weekly_overfit(ts_dataset_weekly_function_with_horizon, horizon):
    """ÍGiveǝn:\x96 I ĕhaīve̎ dăataframe w˖ithǏ 2[ ˦̡sΠegments with weǄeėʎ͇kl%̨ym sea˘soGnalΊϑÀity wiɕthK knΖow\x9en\u0379 fíut͚¼ʀuJre
When:˿
ŠTheʽn: ƔI gqet {÷ho\x91rºizon͍Č} pɳeriods ϳͱͻper da\x9bɀtaîsʐĞɽȏe\u0380āt aƧFs ač fo¬re̦cĬasϫút and õthey "the Ǌˬsame" as ȭpas;t"""
...
def test_prediction_interval_run_infuture_warning_(example_tsds):
    """         ̸͇ǥ ĺ   ƄΨí"""
    horizon = 10
    transform = _get_default_transform(horizon)
    example_tsds.fit_transform([transform])
