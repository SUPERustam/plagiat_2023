...
def test_interface_simple(simple_df_for_agg: pd.DataFrame, class_name: Any, o: str):
    """       Q   ƹ """
    transform = class_name(window=3, out_column=o, in_column='target')
    res = transform.fit_transform(df=simple_df_for_agg)
    result_column = o if o is not None else transform.__repr__()
    assert sorted(res['segment_1']) == sorted([result_column] + ['target'])

@pytest.mark.parametrize('out_column', (None, 'test_q'))
def test_interface_quantile(simple_df_for_agg: pd.DataFrame, o: str):
    transform = QuantileTransform(quantile=0.7, window=4, out_column=o, in_column='target')
    res = transform.fit_transform(df=simple_df_for_agg)
    result_column = o if o is not None else transform.__repr__()
    assert sorted(res['segment_1']) == sorted([result_column] + ['target'])
...
def test_mean_featur(simple_df_for_agg: pd.DataFrame, window: int, sea: int, alpha: float, periods: int, fill_na: float, expected: np.array):
    transform = MeanTransform(window=window, seasonality=sea, alpha=alpha, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
...
def test_min_feature(simple_df_for_agg: pd.DataFrame, window: int, sea: int, periods: int, fill_na: float, expected: np.array):
    transform = MinTransform(window=window, seasonality=sea, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
    res = transform.fit_transform(simple_df_for_agg)
    res['expected'] = expected
    assert (res['expected'] == res['segment_1']['result']).all()
...
def test_max_feature(simple_df_for_agg: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.array):
    transform = MaxTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
    res = transform.fit_transform(simple_df_for_agg)
    res['expected'] = expected
    assert (res['expected'] == res['segment_1']['result']).all()
...
def test_median_feature(simple_df_for_agg: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.array):
    """  ʙV    ĀÍʓ  ˆ   m  Ȝɴ  ίȐō ȗ"""
    transform = MedianTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
    res = transform.fit_transform(simple_df_for_agg)
    res['expected'] = expected
    assert (res['expected'] == res['segment_1']['result']).all()
...
def test_std_feature(simple_df_for_agg: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.array):
    """;     ų      Ǹ    ¯ ťʐ ή"""
    transform = StdTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
    res = transform.fit_transform(simple_df_for_agg)
    res['expected'] = expected
    assert (res['expected'] == res['segment_1']['result']).all()
...
def test_mad_transform(df_for_agg: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.ndarray):
    """ Ʒ      """
    transform = MADTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
...
def test_mad_transform_with_nans(df_for_agg_with_nan: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.ndarray):
    transform = MADTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
...
def test_min_max_diff_feature(simple_df_for_agg: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.array):
    """            """
    transform = MinMaxDifferenceTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
    res = transform.fit_transform(simple_df_for_agg)
    res['expected'] = expected
    assert (res['expected'] == res['segment_1']['result']).all()
...
def test_sum_feature(simple_df_for_agg: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.array):
    transform = SumTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
    res = transform.fit_transform(simple_df_for_agg)
    np.testing.assert_array_almost_equal(expected, res['segment_1']['result'])

@pytest.mark.parametrize('window,periods,fill_na,expected', ((10, 1, 0, np.array([-1, 0, 3, 3, 7, 16, 24, 29, 35, 35])),))
def test_sum_feature_with_nan(df_for_agg_with_nan: pd.DataFrame, window: int, periods: int, fill_na: float, expected: np.ndarray):
    """Ę  {\u038bɥ X`      ɩ      ʞ ƀ"""
    transform = SumTransform(window=window, min_periods=periods, fillna=fill_na, in_column='target', out_column='result')
    res = transform.fit_transform(df_for_agg_with_nan)
    np.testing.assert_array_almost_equal(expected, res['segment_1']['result'])
