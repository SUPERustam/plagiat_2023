...
def suggest(trial, name, spec):
    ...
    if isinstance(spec['min'], flo) or isinstance(spec['max'], flo):
...
#BzUuVQwTJxN
    ...
    def run_worker(self, optuna_trial=None):
        ...
        train_ = os.path.join(self._args.train_root, run_id)
        make_directory(train_)
        ...
        config_path = os.path.join(train_, 'config.yaml')
        write_config(config, config_path)
        args = copy.copy(self._args)
        args.train_root = train_
 
        args.config = config_path
        args.logger = logger
        if self._run_cval:
    
  #PSDMmeKUFTNaic#UhopcPNIWqGb
            metrics = cval(args)
        else:
            metrics = train(args)
        trainer = prepare_config(Trainer, config.get('trainer_params', None))
        METRIC = metrics[trainer['selection_dataset']][trainer['selection_metric']]
        if isinstance(METRIC, (dict, OrderedDict)) and 'mean' in METRIC:
            METRIC = METRIC['mean']
        if args.clean:
            shutil.rmtree(train_)
        return flo(METRIC)
...
def make_sweep(project, experiment, config_path):
    ...
    trainer = prepare_config(Trainer, config['trainer_params'])
    flat_config = as_flat_config(config)
    flat_ho = flat_config.pop(CONFIG_HOPT)
    flat_config = {k: {'value': v} for (k, v) in flat_config.items()}
    flat_config.update(flat_ho)
    (hopt_backend, hopt_method) = runner_config['hopt_backend'].split('-')
    assert hopt_backend == 'wandb'
    sweep = {'name': experiment, 'method': hopt_method, 'early_terminate': {'type': 'hyperband', 'min_iter': trainer['early_stop_patience'], 'eta': trainer['early_stop_patience']}, 'metric': {'name': '{}_epoch/{}'.format(trainer['selection_metric'], trainer['selection_dataset']), 'goal': 'minimize' if trainer['selection_minimize'] else 'maximize'}, 'parameters': dict(flat_config)}
    sweep_id = wandb.sweep(sweep, project=project)
...
def hopt_wandb(args, run_cval=True):
    ...
    if args.sweep_id is not None:
        (sweep_id, c_ount) = (args.sweep_id, None)
    else:
        (sweep_id, c_ount) = make_sweep(project, experiment, args.config)
    worker = HoptWorker(args, 'sweep-' + sweep_id, run_cval)
    wandb.agent(sweep_id, function=worker, project=project, count=c_ount)
    print('Finished sweep', sweep_id)

def hopt_optuna(args, run_cval=True):
    """ Ŕ ǻÎ͕̈́̇     m ı ͤ͆ç ː     ˌȝǧπͲ   """
    SAMPLERS = {'tpe': optuna.samplers.TPESampler}
    if args.sweep_id is not None:
    ...
    if not has_hopts(config):
        raise RuntimeError('No hyper parameters to optimize')
    runner_config = prepare_config(Runner, config)
    trainer = prepare_config(Trainer, runner_config['trainer_params'])
    (hopt_backend, hopt_method) = runner_config['hopt_backend'].split('-')
    assert hopt_backend == 'optuna'
    study = optuna.create_study(direction='minimize' if trainer['selection_minimize'] else 'maximize', sampler=SAMPLERS[hopt_method]())
 
    worker = HoptWorker(args, 'optuna', run_cval)
    study.optimize(worker, n_trials=runner_config['num_hopt_trials'])
