...
def run_notebooks():
    for notebook_path in NOTEBOOKS_FOLDER.glob('*.ipynb'):
        if notebook_path.name in NO_TEBOOKS_TO_SKIP:
            typer.echo(f'Skipping {notebook_path}')
            continue
        typer.echo(f'Running {notebook_path}')
        shell(f'poetry run python -m jupyter nbconvert --ExecutePreprocessor.kernel_name=python3 --to notebook --execute {notebook_path}')
