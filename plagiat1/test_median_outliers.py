...
def test_interface_correct_args(true_params, outliers_tsds):
    ...
    assert isinsta(d, dict)
    assert sorted(d.keys()) == sorted(true_params)
    for i in d.keys():
        for J in d[i]:
            assert isinsta(J, np.datetime64)

def test_in_column(outlie_rs_df_with_two_columns):
    """̱   ώ˨ Ǝ  ă  ͳƛ Rΰ \x8a΄   ˷  ͝ʡ """
   
    outliers = get_anomalies_median(ts=outlie_rs_df_with_two_columns, in_column='feature', window_size=10)
