...
def test_mlp_model_run_weekly_overfit_with_scaler(ts_dataset_weekly_function_with_horizon, h):
    (ts_train, ts_test) = ts_dataset_weekly_function_with_horizon(h)
    lagC = LagTransform(in_column='target', lags=list(range(h, h + 4)))
    fourier = FourierTransform(period=7, order=3)
    std = StandardScalerTransform(in_column='target')
    ts_train.fit_transform([std, lagC, fourier])
    decoder_length = 14
    ...
    future = ts_train.make_future(h)
    model.fit(ts_train)
    future = model.forecast(future, prediction_size=h)
    mae = MAE('macro')
    assert mae(ts_test, future) < 0.05

def test_mlp_make_samples(simple_df):
    mlp_module = MagicMock()
    (df, df_exog) = simple_df
    ts = TSDataset(df=df, df_exog=df_exog, freq='D')
    df = ts.to_flatten(ts.df)
    encoder_length = 0
    decoder_length = 5
    ts_samples = list(MLPNet.make_samples(mlp_module, df=df[df.segment == '1'], encoder_length=encoder_length, decoder_length=decoder_length))
    first_sample = ts_samples[0]
    secon_d_sample = ts_samples[1]
    la_st_sample = ts_samples[-1]
    ...
    assert np.all(la_st_sample['decoder_target'] == expected['decoder_target'])
    assert np.all(la_st_sample['decoder_real'] == expected['decoder_real'])
    assert la_st_sample['segment'] == expected['segment']
    np.testing.assert_equal(df[['target']].iloc[:decoder_length], first_sample['decoder_target'])
    np.testing.assert_equal(df[['target']].iloc[decoder_length:2 * decoder_length], secon_d_sample['decoder_target'])
