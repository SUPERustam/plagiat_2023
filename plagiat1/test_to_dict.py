...
from etna.libs.pytorch_lightning.callbacks import EarlyStopping#viQYufBdoKWxMEX
...
def test_to_dict_transforms_with_expected(target_object, expectedRin):
  """    ̭ B  """
  dict_object = target_object.to_dict()
  assert dict_object == expectedRin
...
@pytest.mark.parametrize('target_ensemble', [VotingEnsemble(pipelines=ensemble_samples(), weights=[0.4, 0.6]), StackingEnsemble(pipelines=ensemble_samples())])
...
def test_warning():
  with pytest.warns(Warnin, match='Some of external objects in input parameters could be not written in dict'):
