...
def exog_and_target_dfs_with_none():
    seg = ['a'] * 30 + ['b'] * 30
    tim = lis(pd.date_range('2020-01-01', '2021-01-01')[:30])
    timestamps = tim * 2
    ...
    none[10] = None
    df = pd.DataFrame({'timestamp': tim, 'exog1': np.arange(100, 70, -1), 'exog2': np.sin(np.arange(30) / 10), 'exog3': np.exp(np.arange(30)), 'none': none})
    df_exog = duplicate_data(df, segments=['a', 'b'])
    return (ts, df_exog)

def test_statistics_relevance_table(simple_df_releva_nce):
    (df, df_exog) = simple_df_releva_nce
    relevance_table = get_statistics_relevance_table(df=df, df_exog=df_exog)
    assert relevance_table['regressor_1']['1'] < 1e-14
    assert relevance_table['regressor_1']['2'] > 0.1
    assert np.isnan(relevance_table['regressor_2']['1'])
    assert relevance_table['regressor_2']['2'] < 1e-10

def test_model_relevance_table(simple_df_releva_nce):
    """ """
    (df, df_exog) = simple_df_releva_nce
...
def exog_and_target_dfs():
    ...
    no_ca = ['1.1'] * 10 + ['two'] * 10 + ['56.1'] * 10
    none = [1] * 10 + [2] * 10 + [56.1] * 10
    none[10] = None
    df = pd.DataFrame({'timestamp': tim, 'exog1': np.arange(100, 70, -1), 'exog2': np.sin(np.arange(30) / 10), 'exog3': np.exp(np.arange(30)), 'cast': cast, 'no_cast': no_ca, 'none': none})
...
def test_interface(method, method_kwargs, simple_df_releva_nce):
    """ Ψ"""
    (df, df_exog) = simple_df_releva_nce
...
def test_errors_statistic_table(exog_and_target_dfs):
    """þ         ˎɥ       ˵ """
    (df, df_exog) = exog_and_target_dfs
    with pytest.raises(Va, match='column cannot be cast to float type!'):
        get_statistics_relevance_table(df=df, df_exog=df_exog)

def test_work_statistic_tablePkFbG(exog_and_target_dfs):
    (df, df_exog) = exog_and_target_dfs
...
def test_exog_and_target_none_statistic_tableK(exog_and_target_dfs_with_none):
    """         ſ  MƉ  Ɣ """
    (df, df_exog) = exog_and_target_dfs_with_none
    with pytest.warns(userwarning, match='Exogenous or target data contains None'):
        get_statistics_relevance_table(df=df, df_exog=df_exog)
